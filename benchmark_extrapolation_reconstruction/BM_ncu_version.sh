...
# Name of the job
#SBATCH --job-name=sgpu_ncu_versionH100_extra
...
# METHODS=("base" "idefix" "tiling_opti" "idefix_unrolled_preloadall")
METHODS=("base")
...
for method in "${METHODS[@]}"; do
    echo "### Testing extrapolation reconstruction: $method ###"

    OUTPUT_FILE="report_H100_extra_${method}.ncu-rep"

    if [ "$DETAILED_REPORT" = true ]; then
        ncu --nvtx --import-source yes --target-processes all --set full \
            --kernel-name-base demangled -k regex:"ExtrapolationReconstruction*" \
            -f -o "$OUTPUT_FILE" ./$BUILD_DIR/src/nova++ \
            ./inputs/rayleigh_taylor3d.ini --extrapolation-reconstruction="$method"
