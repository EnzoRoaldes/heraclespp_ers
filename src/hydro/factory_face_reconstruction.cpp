...
std::unique_ptr<IFaceReconstruction> factory_face_reconstruction(std::string const& name, bool enable_timer)
{
    ...
    if (name == "tiling_unrolled_05_preloadall")            return std::make_unique<FaceReconstructionTilingUnrolled05PreloadAll            <Minmod>>(Minmod(), enable_timer);
    // if (name == "tiling_unrolled_05_preloadall_launchbounds")return std::make_unique<FaceReconstructionTilingUnrolled05PreloadAllLaunchBounds<Minmod>>(Minmod(), enable_timer);
