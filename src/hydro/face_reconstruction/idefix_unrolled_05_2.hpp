...
class FaceReconstructionIdefixUnrolled052 : public IFaceReconstruction
{
...
public:
    explicit FaceReconstructionIdefixUnrolled052(SlopeLimiter limiter, bool enable_timer = false) 
        ...
        KV_double_5d const& var_rec) const override
    {
        ...
        if (m_enable_timer) {
                ...
                KOKKOS_LAMBDA(int i, int j, int k)
            {
                ...
                {                
                    double const slope = slope_limiter(      
                        2.0 * (var(i+1, j, k) - var(i, j, k)) / (dx(i) + dx(i+1)),
                        2.0 * (var(i, j, k) - var(i-1, j, k)) / (dx(i-1) + dx(i)));
                ...
                {
                    double const slope = slope_limiter(      
                        2.0 * (var(i, j+1, k) - var(i, j, k)) / (dy(j) + dy(j+1)),
                        2.0 * (var(i, j, k) - var(i, j-1, k)) / (dy(j-1) + dy(j)));
                ...
                {
                    double const slope = slope_limiter(      
                        2.0 * (var(i, j, k+1) - var(i, j, k)) / (dz(k) + dz(k+1)),
                        2.0 * (var(i, j, k) - var(i, j, k-1)) / (dz(k-1) + dz(k)));
            ...
            if (timing_file) {
                timing_file << "idefix_unrolled_05_2" << " " << ms << "\n";
            }
