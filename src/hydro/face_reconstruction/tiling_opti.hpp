...
class FaceReconstructionTilingOpti : public IFaceReconstruction
{
...
public:
    explicit FaceReconstructionTilingOpti(SlopeLimiter limiter, bool enable_timer = false) 
        ...
        KV_double_5d const& var_rec) const override
    {
        ...
        std::array<int, 3> m_tiling = {32, 2, 1}; // Best tiling
        ...
        if (m_enable_timer) {
            ...
            if (timing_file) {
                timing_file << "tiling_opti" << " " << ms << " " << m_tiling[0] << " " << m_tiling[1] << " " << m_tiling[2] << "\n";
            }
