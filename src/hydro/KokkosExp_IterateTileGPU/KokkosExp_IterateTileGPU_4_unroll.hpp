...
// KokkosExp_IterateTileGPU_v4.hpp
...
struct DeviceIterateTile<3, PolicyType, Functor, MaxGridSize, Tag> {
  ...
  void exec_range() const {
    ...
    if constexpr (PolicyType::inner_direction == Iterate::Left) {
        ...
        const index_type lower0 = m_policy.m_lower[0];
        const index_type lower1 = m_policy.m_lower[1];
        const index_type lower2 = m_policy.m_lower[2];
        const index_type upper0 = m_policy.m_upper[0];
        const index_type upper1 = m_policy.m_upper[1];
        const index_type upper2 = m_policy.m_upper[2];
        const index_type tile0  = m_policy.m_tile[0];
        const index_type tile1  = m_policy.m_tile[1];
        const index_type tile2  = m_policy.m_tile[2];
        const index_type tix    = static_cast<index_type>(threadIdx.x);
        const index_type tiy    = static_cast<index_type>(threadIdx.y);
        const index_type tiz    = static_cast<index_type>(threadIdx.z);
        const index_type gdx    = static_cast<index_type>(gridDim.x);
        const index_type gdy    = static_cast<index_type>(gridDim.y);
        const index_type gdz    = static_cast<index_type>(gridDim.z);
        const index_type bix    = static_cast<index_type>(blockIdx.x);
        const index_type biy    = static_cast<index_type>(blockIdx.y);
        const index_type biz    = static_cast<index_type>(blockIdx.z);
        // iterate over z blocks
        #pragma unroll 1
        for (index_type tile_id2 = biz; tile_id2 < m_policy.m_tile_end[2]; tile_id2 += gdz) {
            // check index for dimension 2 is within range
            // cheap checks first
            if (tiz >= tile2) continue;
            const index_type offset_2 = tile_id2 * tile2 + tiz + lower2;
            if (offset_2 >= upper2) continue;

            // iterate over y blocks
            #pragma unroll 1
            for (index_type tile_id1 = biy; tile_id1 < m_policy.m_tile_end[1]; tile_id1 += gdy) {
                // check index for dimension 1 is within range
                // cheap checks first
                if (tiy >= tile1) continue;
                // compute index for dimension 1
                const index_type offset_1 = tile_id1 * tile1 + tiy + lower1;
                if (offset_1 >= upper1) continue;

                // iterate over x blocks
                #pragma unroll 1
                for (index_type tile_id0 = bix; tile_id0 < m_policy.m_tile_end[0]; tile_id0 += gdx) {
                    // check index for dimension 0 is within range
                    if (tix >= tile0) continue;
                    const index_type offset_0 = tile_id0 * tile0 + tix + lower0;
                    if (offset_0 >= upper0) continue;
                        // call kernel with computed indices
