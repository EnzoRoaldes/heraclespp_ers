...
class HancockExtrapolationReconstructionBase : public IExtrapolationReconstruction<UniformGravity>
{
private:
    thermodynamics::PerfectGas m_eos;

public:
    explicit HancockExtrapolationReconstructionBase(thermodynamics::PerfectGas const& eos)
        : m_eos(eos) {}

    void execute(
        Range const& range,
        Grid const& grid,
        UniformGravity const& gravity,
